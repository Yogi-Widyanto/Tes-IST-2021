                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <h3 class="m-0 font-weight-bold text-primary text-center">
                                Edit Tanggal Tes Pada Tes IST 2021
                            </h3>
                            <hr class="sidebar-divider d-none d-md-block">
                            <form action="{{route('Updatetanggal.tes')}}" method="post">
                                @csrf
                                @foreach ($data as $item)
                                <input type="hidden" name="id" value="{{ $item->id_tanggalTes }}"> <br/>
                                <div class="form-group">
                                    <label for="ket">Keterangan Tes</label>
                                    <input id="ket" class="form-control" type="text" name="ket" value="{{$item->keterangan_tes}}">
                                </div>

                                <div class="form-group">
                                    <label for="tgl1">Tanggal Mulai</label>
                                    <input id="tgl1" class="form-control" type="date" name="tgl1" value="{{$item->tglMulai}}">
                                </div>

                                <div class="form-group">
                                    <label for="tgl2">Tanggal Hasil</label>
                                    <input id="tgl2" class="form-control" type="date" name="tgl2" value="{{$item->tglHasil}}">
                                </div>
                                <div class="form-group">
